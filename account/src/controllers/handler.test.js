const { deleteOne, updateOne, createOne, getOne, getAll } = require('./handler.controller');
...
describe('Controller Functions', () => {
  ...
  describe('deleteOne', () => {
    ...
    it('should call next with an error if no document is found', async () => {
      ...
      expect(next.mock.calls[0][0].message).toBe('No document found with that ID');
  ...
  describe('updateOne', () => {
    it('should update a document and return 200', async () => {
      ...
      expect(Model.findByIdAndUpdate).toHaveBeenCalledWith('123', req.body, { new: true, runValidators: true });
  ...
  describe('getAll', () => {
    it('should return all documents and return 200', async () => {
      ...
      const docs = [{ id: '1', name: 'Doc1' }, { id: '2', name: 'Doc2' }];
